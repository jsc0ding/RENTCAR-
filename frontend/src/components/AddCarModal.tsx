...
const AddCarModal = ({ onCarAdded }: AddCarModalProps) => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!formData.name || !formData.brand || !formData.model || !formData.year || 
        !formData.price || !formData.category || !formData.passengers || 
        !formData.transmission || !formData.fuel || !formData.engine || 
        !formData.color || !formData.mileage || !formData.location) {
    ...
    try {
      ...
      const carData = {
        ...formData,
        features: formData.features.join(',').split(',').map((f: string) => f.trim()).filter((f: string) => f)
      };
      ...
      if (response.success) {
        ...
        toast.error(response.message || "Mashina qo'shishda xatolik", {
          position: "top-center",
          duration: 3000,
        });
      }
    ...
    <Dialog open={open} onOpenChange={setOpen}>
      <DialogTrigger asChild>
        <Button className="bg-gradient-to-r from-primary to-accent hover:opacity-90">
          ...
          <span className="hidden sm:inline">Yangi mashina</span>
